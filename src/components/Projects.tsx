...
`.trim(),
...

    {
        ...
        troubleshooting: `
**문제:** 의뢰인이 입력 정보의 오류나 중복 신청 발생을 우려하여, 데이터의 신뢰성과 관리 효율성을 높일 수 있는 기능이 필요함.

**해결**

• 전화번호, 생년월일 등 입력값에 자동 포맷팅과 유효성 검사를 적용해 잘못된 입력을 사전에 차단함.

• 데이터베이스 설계 단계에서 중복 신청 방지를 위해 고유 키(primary key, unique 설정)를 적용하여 데이터 무결성을 확보함.

• Next.js의 API Routes와 Supabase, xlsx 라이브러리를 활용해 실시간 모니터 및 엑셀 다운로드가 가능한 관리자 페이지를 제작.
`.trim(),
